...
try {
  ...
  try {
    ...
    try {
      ...
      await backupPage.getByRole("status").filter({ hasText: "Workspace backup restored locally." }).waitFor({ state: "visible" });
      await backupPage.goto(`${baseUrl}/forms`, { waitUntil: "networkidle" });
      if (await backupPage.locator(".question-label-input").count() !== 0 || await backupPage.getByText("Build a form that fits the moment.").count() !== 1 || (await backupPage.locator("body").innerText()).includes("What are you working on?")) failures.push({ route: "settings", controls: "partial backup stays blank" });
      await backupPage.goto(`${baseUrl}/home`, { waitUntil: "networkidle" });
      if (!(await backupPage.locator(".workspace-list").innerText()).includes("Research") || !(await backupPage.locator(".project-list").innerText()).includes("Migration")) failures.push({ route: "settings", controls: "legacy workspace/project backup normalization" });
      await backupPage.getByRole("textbox", { name: "Search across Crescent" }).fill("Research");
      if (await backupPage.locator(".search-results .search-result").count() !== 1) failures.push({ route: "home", controls: "workspace/project global search" });
      await backupPage.getByRole("textbox", { name: "Search across Crescent" }).fill("Migration");
      await backupPage.getByRole("textbox", { name: "Search across Crescent" }).press("Enter");
      await backupPage.waitForFunction(() => document.querySelector(".task-project-filter")?.value?.toLowerCase() === "migration");
      if (backupPage.url().split("#")[1] !== "tasks" || (await backupPage.getByRole("combobox", { name: "Filter tasks by project" }).inputValue()).toLowerCase() !== "migration") failures.push({ route: "search", controls: "open exact Project result", url: backupPage.url() });
      await backupPage.goto(`${baseUrl}/drive`, { waitUntil: "networkidle" });
      const migratedFolders = await backupPage.locator(".folder-grid").innerText();
      ...
      await backupPage.goto(`${baseUrl}/home`, { waitUntil: "networkidle" });
      await backupPage.getByRole("textbox", { name: "Search across Crescent" }).fill("Smoke restore");
      await backupPage.getByRole("textbox", { name: "Search across Crescent" }).press("Enter");
      await backupPage.waitForFunction(() => globalThis.location.hash === "#docs");
      if (await backupPage.getByRole("textbox", { name: "File title" }).inputValue() !== "Smoke restore") failures.push({ route: "drive", controls: "exact partial-backup document destination" });
    } finally {
    ...
    await ensureGlobalChrome();
    await behaviorPage.getByRole("textbox", { name: "Search across Crescent" }).fill("How clear is the next step?");
    await behaviorPage.getByRole("textbox", { name: "Search across Crescent" }).press("Enter");
  ...
  const firstRunContext = await browser.newContext({ viewport: viewports[0] });
  try {
    const firstRunPage = await firstRunContext.newPage();
    await firstRunPage.goto(`${baseUrl}/home?fresh=1`, { waitUntil: "networkidle" });
    await firstRunPage.getByRole("button", { name: "Start with Docs" }).click();
    await firstRunPage.waitForURL(/#docs$/);
    await firstRunPage.getByRole("textbox", { name: "File title" }).fill("First-run plan");
    await firstRunPage.getByRole("textbox", { name: "Document body" }).fill("A useful local document created from the blank workspace.");
    await firstRunPage.getByRole("textbox", { name: "Document body" }).press("Tab");
    await firstRunPage.waitForTimeout(250);
    await firstRunPage.getByRole("button", { name: "Exit focus mode" }).click();
    await firstRunPage.getByRole("button", { name: "Back to Home" }).click();
    await firstRunPage.waitForURL(/#home$/);
    if (await firstRunPage.locator(".recent-row").filter({ hasText: "First-run plan" }).count() !== 1) failures.push({ route: "first-run", controls: "Docs creation appears in Home Recent" });
