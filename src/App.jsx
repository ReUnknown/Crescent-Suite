...
function SheetsView({ workspace, update, onNavigate }) {
  ...
  const persistCells = (nextCells) => { const nextTabs = sheetTabs.map((sheet, index) => index === activeSheet ? { ...sheet, cells: nextCells } : sheet); update({ sheets: { ...workspace.sheets, tabs: nextTabs, activeSheet, cells: nextCells, updatedAt: "just now" } }); };
  ...
  const addSheet = () => { const nextTabs = [...sheetTabs, { name: `Sheet ${sheetTabs.length + 1}`, cells: {} }]; update({ sheets: { ...workspace.sheets, tabs: nextTabs, activeSheet: nextTabs.length - 1, cells: {}, updatedAt: "just now" } }); setSelected("A1"); setView("grid"); };
