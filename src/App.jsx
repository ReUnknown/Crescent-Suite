...
function TasksView({ workspace, update, onNavigate }) {
  ...
  const removeTask = (id) => { const task = workspace.tasks.find((item) => item.id === id); if (!task) return; update({ tasks: workspace.tasks.filter((item) => item.id !== id), deletedFiles: [{ id: `task-${id}`, taskId: id, title: task.title, type: "Tasks", appId: "tasks", opened: "just now", owner: "Me" }, ...(workspace.deletedFiles ?? [])] }); emitNotice("Task moved to local Trash."); };
...
function UtilityView({ id, workspace, update, onNavigate }) {
  ...
  const restoreFile = (file) => { if (file.type !== "Tasks") return; const restoredTask = { id: file.taskId ?? Date.now(), title: file.title, project: "Recovered", due: "Today", complete: false }; update({ tasks: [restoredTask, ...workspace.tasks], deletedFiles: (workspace.deletedFiles ?? []).filter((item) => item.id !== file.id) }); emitNotice("Task restored to Tasks."); };
