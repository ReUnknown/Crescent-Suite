...
function TasksView({ workspace, update, onNavigate }) {
  ...
  const removeTask = (id) => { const task = workspace.tasks.find((item) => item.id === id); if (!task) return; update({ tasks: workspace.tasks.filter((item) => item.id !== id), deletedFiles: [{ id: `task-${id}`, title: task.title, type: "Tasks", appId: "tasks", opened: "just now", owner: "Me" }, ...(workspace.deletedFiles ?? [])] }); emitNotice("Task moved to local Trash."); };
...
function UtilityView({ id, workspace, update, onNavigate }) {
  ...
  const files = id === "starred" ? RECENT_FILES.filter((file) => file.starred) : id === "shared" ? RECENT_FILES.filter((file) => file.owner !== "Me") : id === "trash" ? (workspace.deletedFiles ?? []).map((file) => ({ ...file, ...(APP_META.find((app) => app.id === file.appId) ?? {}) })) : getLiveRecentFiles(workspace);
