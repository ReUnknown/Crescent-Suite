...
function DriveView({ workspace, update, onNavigate, initialFolderName, initialFileTitle }) {
  ...
  const createFile = () => { const typeInput = window.prompt("File type (Docs, Sheets, Slides, Notes, Forms)", "Docs"); if (!typeInput?.trim()) return; const type = ({ docs: "Docs", sheets: "Sheets", slides: "Slides", notes: "Notes", forms: "Forms" })[typeInput.trim().toLowerCase()]; if (!type) { emitNotice("Choose Docs, Sheets, Slides, Notes, or Forms."); return; } const defaults = { Docs: "Untitled document", Sheets: "Untitled spreadsheet", Slides: "Untitled presentation", Notes: "Untitled note", Forms: "Untitled form" }; const requestedTitle = window.prompt(`${type} name`, defaults[type]); if (!requestedTitle?.trim()) return; const title = requestedTitle.trim(); const starredTitles = getStarredTitles(workspace); const deletedFiles = [...(workspace.deletedFiles ?? [])]; const archive = (record) => { starredTitles.delete(record.title); deletedFiles.unshift(record); }; if (type === "Docs") { const previous = workspace.docs; archive({ id: `doc-${Date.now()}`, title: previous.title, body: previous.body, updatedAt: previous.updatedAt, type, appId: "docs", opened: "just now", owner: "Me", starred: getStarredTitles(workspace).has(previous.title) }); update({ docs: { title, body: `<h1>${escapeHtml(title)}</h1><p>Start writing your next idea here.</p>`, updatedAt: "just now" }, starredFiles: [...starredTitles], deletedFiles }); } else if (type === "Sheets") { const previous = workspace.sheets; archive({ id: `sheet-${Date.now()}`, title: previous.title, cells: previous.cells, tabs: previous.tabs, styles: previous.styles, activeSheet: previous.activeSheet, type, appId: "sheets", opened: "just now", owner: "Me", starred: getStarredTitles(workspace).has(previous.title) }); const cells = { A1: "" }; update({ sheets: { title, cells, tabs: [{ name: "Sheet 1", cells }], activeSheet: 0, styles: {}, updatedAt: "just now" }, starredFiles: [...starredTitles], deletedFiles }); } else if (type === "Slides") { const previousTitle = workspace.slidesTitle; archive({ id: `slides-${Date.now()}`, title: previousTitle, slides: workspace.slides, type, appId: "slides", opened: "just now", owner: "Me", starred: getStarredTitles(workspace).has(previousTitle) }); update({ slidesTitle: title, slides: [{ title: "A new chapter", body: "Add a thought worth sharing.", accent: "blue", notes: "", layout: "title-body" }], starredFiles: [...starredTitles], deletedFiles }); } else if (type === "Forms") { const previousTitle = workspace.formTitle; archive({ id: `form-${Date.now()}`, title: previousTitle, formTitle: previousTitle, formTheme: workspace.formTheme, formSettings: workspace.formSettings, forms: workspace.forms, formPublished: workspace.formPublished, formResponses: workspace.formResponses, lastFormResponse: workspace.lastFormResponse, type, appId: "forms", opened: "just now", owner: "Me", starred: getStarredTitles(workspace).has(previousTitle) }); update({ formTitle: title, formTheme: "lilac", formSettings: { ...INITIAL_WORKSPACE.formSettings }, forms: [...INITIAL_WORKSPACE.forms], formPublished: false, formResponses: [], lastFormResponse: undefined, starredFiles: [...starredTitles], deletedFiles }); } else { const newNote = { id: Date.now(), title, body: "Start writing...", color: "blue", updatedAt: "just now" }; update({ notes: [newNote, ...(workspace.notes ?? [])] }); } emitNotice(`${type} created in this local workspace.`); onNavigate(type.toLowerCase(), { title }); };
