...
function DriveView({ workspace, update, onNavigate }) {
  ...
  const createFile = () => { const requestedTitle = window.prompt("Document name", "Untitled document"); if (!requestedTitle?.trim()) return; const title = requestedTitle.trim(); const previousDoc = workspace.docs; const deletedDoc = { id: `doc-${Date.now()}`, title: previousDoc.title, body: previousDoc.body, updatedAt: previousDoc.updatedAt, type: "Docs", appId: "docs", opened: "just now", owner: "Me" }; const starredTitles = getStarredTitles(workspace); starredTitles.delete(previousDoc.title); update({ docs: { title, body: `<h1>${escapeHtml(title)}</h1><p>Start writing your next idea here.</p>`, updatedAt: "just now" }, starredFiles: [...starredTitles], deletedFiles: [deletedDoc, ...(workspace.deletedFiles ?? [])] }); onNavigate("docs"); };
...
function UtilityView({ id, workspace, update, onNavigate }) {
  ...
  const restoreFile = (file) => {
    if (file.type === "Docs") {
      ...
      update({ docs: restoredDoc, deletedFiles: [archivedCurrent, ...(workspace.deletedFiles ?? []).filter((item) => item.id !== file.id)] });
