...
function SearchResults({ query, onNavigate, workspace }) {
  ...
  const searchTextFor = (file) => file.type === "Docs" ? workspace.docs?.body?.replace(/<[^>]+>/g, " ") ?? "" : file.type === "Notes" ? workspace.notes?.find((note) => note.title === file.title)?.body ?? "" : file.type === "Forms" ? (workspace.forms ?? []).map((question) => question.label).join(" ") : "";
