...
function Sidebar({ activeApp, onNavigate, open, onClose, workspace, update, collapsed }) {
  ...
  const openAddDialog = (kind) => { setAddDialog(kind); setAddName(""); };
  useEffect(() => {
  ...
  return <>
    ...
    <aside className={`sidebar ${open ? "sidebar-open" : ""} ${collapsed ? "sidebar-collapsed" : ""}`}>
      ...
      <div className="project-list">
        ...
        {!projects.length && <button className="project-link project-empty-action" onClick={() => openAddDialog("project")} title="Add your first project" aria-label="Add your first project"><Plus size={16} /><span>Add your first project</span></button>}
      </div>
...
function MailView({ workspace, update, onNavigate, initialSubject }) {
  ...
  const sendMessage = () => {
    ...
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(to)) { emitNotice("Use an email address like name@example.com."); return; }
...
function NotesView({ workspace, update, onNavigate, initialNoteTitle }) {
  ...
  const addNote = () => { const id = Date.now(); update({ notes: [{ id, title: "", body: "", color: "blue", updatedAt: "Just now" }, ...workspace.notes] }); setSelected(id); };
...
function DriveView({ workspace, update, onNavigate, initialFolderName, initialFileTitle }) {
  ...
  const openFolderDialog = () => { setFolderName(""); setFolderDialog(true); };
  ...
  const openCreateFile = () => { setNewFileType("Docs"); setNewFileTitle(""); setNewFileDialog(true); };
  const selectNewFileType = (type) => { setNewFileType(type); setNewFileTitle(""); };
  const newFileTitlePlaceholder = { Docs: "Name this document", Sheets: "Name this spreadsheet", Slides: "Name this presentation", Notes: "Name this note", Forms: "Name this form" }[newFileType] ?? "Name this file";
  useEffect(() => {
...
function FormsView({ workspace, update, onNavigate, initialQuestionId }) {
  ...
  const addQuestion = () => update({ forms: [...workspace.forms, { id: Date.now(), label: "", type: "Short answer", required: false }] });
  ...
  const previewReady = workspace.forms.length > 0 && workspace.forms.every((question) => String(question.label ?? "").trim());
  ...
  return <div className={`forms-page page-enter form-theme-${formTheme}`}>
    <EditorHeader title={formTitle} icon={APP_META.find((app) => app.id === "forms")} onChangeTitle={(value) => { setFormTitle(value); update({ formTitle: value, starredFiles: renameStarredFile(workspace, workspace.formTitle, value) }); }} onNavigate={onNavigate}>
      ...
      {workspace.forms.length > 0 && <button className="secondary-button" onClick={() => { if (!previewReady) { emitNotice("Give every question a label before previewing."); return; } setPreviewing((current) => !current); }} title={previewReady ? "Preview this form" : "Give every question a label before previewing"}><EyeIcon />{previewing ? "Edit form" : "Preview"}</button>}
...
function EyeIcon() { return <span className="eye-icon" aria-hidden="true">◉</span>; }
...
function UtilityView({ id, workspace, update, onNavigate }) {
  ...
  const restoreFile = (file) => {
    if (file.type === "Docs") {
      ...
      const starredTitles = getStarredTitles(workspace);
      const hasCurrentContent = Boolean(String(currentDoc.title ?? "").trim() || String(currentDoc.body ?? "").replace(/<[^>]*>/g, " ").trim());
      ...
      update({ docs: restoredDoc, starredFiles: [...starredTitles], deletedFiles: [...(hasCurrentContent ? [archivedCurrent] : []), ...(workspace.deletedFiles ?? []).filter((item) => item.id !== file.id)] });
      emitNotice("Document restored to Docs.");
      return;
    }
    if (file.type === "Sheets") {
      const currentSheet = workspace.sheets;
      const emptySheet = createEmptyWorkspace().sheets;
      const restoredCells = file.cells && typeof file.cells === "object" ? file.cells : {};
      const restoredTabs = Array.isArray(file.tabs) && file.tabs.length ? file.tabs : [{ name: "Sheet 1", cells: restoredCells }];
      const restoredSheet = { ...emptySheet, title: file.title, cells: restoredCells, tabs: restoredTabs, styles: file.styles && typeof file.styles === "object" ? file.styles : {}, activeSheet: file.activeSheet ?? 0, updatedAt: "Recovered" };
      const starredTitles = getStarredTitles(workspace);
      const hasCurrentContent = Boolean(String(currentSheet.title ?? "").trim() || Object.values(currentSheet.cells ?? {}).some((value) => String(value ?? "").trim()));
      ...
      if (file.starred) starredTitles.add(file.title); else starredTitles.delete(file.title);
      update({ sheets: restoredSheet, starredFiles: [...starredTitles], deletedFiles: [...(hasCurrentContent ? [archivedCurrent] : []), ...(workspace.deletedFiles ?? []).filter((item) => item.id !== file.id)] });
      emitNotice("Spreadsheet restored to Sheets.");
      return;
    }
    if (file.type === "Slides") {
      const currentTitle = workspace.slidesTitle;
      const emptySlides = createEmptyWorkspace().slides;
      const restoredSlides = Array.isArray(file.slides) && file.slides.length ? file.slides : emptySlides;
      const starredTitles = getStarredTitles(workspace);
      const hasCurrentContent = Boolean(String(currentTitle ?? "").trim() || (workspace.slides ?? []).some((slide) => String(slide.title ?? "").trim() || String(slide.body ?? "").trim() || String(slide.notes ?? "").trim()));
      ...
      if (file.starred) starredTitles.add(file.title); else starredTitles.delete(file.title);
      update({ slidesTitle: file.title, slides: restoredSlides, starredFiles: [...starredTitles], deletedFiles: [...(hasCurrentContent ? [archivedCurrent] : []), ...(workspace.deletedFiles ?? []).filter((item) => item.id !== file.id)] });
    ...
    if (file.type === "Forms") {
      ...
      const emptyForm = createEmptyWorkspace();
      const restoredForms = Array.isArray(file.forms) ? file.forms : emptyForm.forms;
      const starredTitles = getStarredTitles(workspace);
      const hasCurrentContent = Boolean(String(currentTitle ?? "").trim() || workspace.forms.length || (workspace.formResponses ?? []).length);
      ...
      starredTitles.delete(currentTitle);
      if (file.starred) starredTitles.add(file.title); else starredTitles.delete(file.title);
      update({ formTitle: file.formTitle ?? file.title, formTheme: file.formTheme ?? "lilac", formSettings: file.formSettings ?? emptyForm.formSettings, forms: restoredForms, formPublished: Boolean(file.formPublished), formResponses: Array.isArray(file.formResponses) ? file.formResponses : [], lastFormResponse: file.lastFormResponse, starredFiles: [...starredTitles], deletedFiles: [...(hasCurrentContent ? [archivedCurrent] : []), ...(workspace.deletedFiles ?? []).filter((item) => item.id !== file.id)] });
...
export default function App() {
  ...
  const currentView = useMemo(() => {
    ...
    if (activeApp === "docs") return <DocsView workspace={workspace} update={update} onNavigate={navigate} initialHeading={navigationContext?.heading} focusMode={focusMode} />;
  ...
  }, [activeApp, focusMode, navigationContext, workspace, update]);
  return <div className={`app-shell ${sidebarCollapsed ? "sidebar-collapsed" : ""} ${focusMode ? "focus-mode" : ""} ${workspace.demo ? "workspace-demo" : "workspace-local"}`}><a className="skip-link" href="#main-content">Skip to content</a><Sidebar activeApp={activeApp} onNavigate={navigate} open={sidebarOpen} onClose={() => setSidebarOpen(false)} workspace={workspace} update={update} collapsed={sidebarCollapsed} /><div className="app-main"><Header activeApp={activeApp} onOpenSidebar={() => setSidebarOpen(true)} onToggleSidebar={() => setSidebarCollapsed((current) => !current)} onOpenGuide={() => setGuideOpen(true)} onToggleFocus={() => setFocusMode((current) => !current)} focusMode={focusMode} sidebarCollapsed={sidebarCollapsed} query={query} onQueryChange={setQuery} onNavigate={navigate} workspace={workspace} /><div className="app-content" id="main-content" tabIndex="-1">{currentView}</div></div>{workspace.demo && <div className="demo-preview-banner" role="status"><Sparkles size={14} /><strong>Demo preview</strong><span>Changes stay in this tab</span></div>}{focusMode && <button className="focus-exit" onClick={() => setFocusMode(false)} aria-label="Exit focus mode" aria-keyshortcuts="Escape" title="Exit focus mode"><PanelLeftOpen size={17} /></button>}<div className={`toast ${notice ? "toast-visible" : ""}`} role="status" aria-live="polite"><CheckCircle2 size={16} />{notice}</div>{guideOpen && <GuideDialog onClose={() => setGuideOpen(false)} onNavigate={navigate} />}</div>;
}
