...
function CalendarView({ workspace, update, onNavigate }) {
  ...
  const localEventsForDate = (date) => localEvents.filter((event) => (event.date ?? displayDate.toISOString().slice(0, 10)) === date.toISOString().slice(0, 10)).map((event) => event.title);
  const monthDays = Array.from({ length: 7 }, (_, index) => { const date = new Date(displayDate); date.setDate(displayDate.getDate() - displayDate.getDay() + index); const seededEvents = index === 2 ? ["Product sync", "Design review", "Focus time"] : index === 3 ? ["Customer interview"] : index === 4 ? ["Launch review"] : []; return { label: new Intl.DateTimeFormat("en-US", { weekday: "short" }).format(date), date: date.getDate(), events: [...seededEvents, ...localEventsForDate(date)] }; });
  const monthCalendar = <div className="calendar-month-grid">{monthDays.map((day) => <div className={`calendar-month-cell ${weekOffset === 0 && day.date === displayDate.getDate() ? "today" : ""}`} key={`${day.label}-${day.date}`}><div className="calendar-month-head"><span>{day.label}</span><strong>{day.date}</strong></div><div className="calendar-month-events">{day.events.map((event, index) => <span className={`calendar-month-event month-event-${index % 4}`} key={`${day.date}-${event}`}>{event}</span>)}</div></div>)}</div>;
  ...
  return <div className="calendar-page page-enter"><EditorHeader title="Calendar" icon={APP_META.find((app) => app.id === "calendar")} onChangeTitle={() => {}} onNavigate={onNavigate}><button className="secondary-button" onClick={exportCalendar} disabled={!localEvents.length}><Download size={16} />Export ICS</button><button className="secondary-button" onClick={() => setWeekOffset((current) => current - 1)}><ChevronLeft size={16} />Previous</button><button className="secondary-button" onClick={() => setWeekOffset(0)}>Today</button><button className="primary-button" onClick={addEvent}><Plus size={16} />Event</button></EditorHeader><div className="calendar-content"><div className="calendar-heading"><div><h1>{mode === "month" ? displayMonth : displayHeading}</h1><p>{mode === "month" ? `${displayMonth} · Product workspace` : mode === "day" ? `${shortDisplayDay} · Product workspace` : `Week of ${displayHeading} · Product workspace`}</p></div><div className="calendar-switch"><button className={mode === "day" ? "selected" : ""} onClick={() => setMode("day")} aria-pressed={mode === "day"}>Day</button><button className={mode === "week" ? "selected" : ""} onClick={() => setMode("week")} aria-pressed={mode === "week"}>Week</button><button className={mode === "month" ? "selected" : ""} onClick={() => setMode("month")} aria-pressed={mode === "month"}>Month</button></div></div>{localEvents.length > 0 && <section className="calendar-local-events" aria-label="Saved local events"><div className="calendar-local-heading"><div><span className="utility-kicker"><CalendarCheck2 size={14} />Saved locally</span><h2>Your Crescent events</h2></div><small>{localEvents.length} event{localEvents.length === 1 ? "" : "s"}</small></div><div className="calendar-local-list">{localEvents.map((event) => <div className="calendar-local-card" key={event.id}><span className="calendar-local-dot" /><div><strong>{event.title}</strong><small>{event.when}</small></div><button className="icon-button muted" onClick={() => removeEvent(event.id)} aria-label={`Remove ${event.title}`}><Trash2 size={15} /></button></div>)}</div></section>}{mode === "month" ? monthCalendar : weekCalendar}</div></div>;
}
...
function FormsView({ workspace, update, onNavigate }) {
  ...
  const exportForm = () => { const exportData = { title: formTitle, questions: workspace.forms, settings: formSettings, response: workspace.lastFormResponse ?? null }; downloadText(`${formTitle || "crescent-form"}.json`, JSON.stringify(exportData, null, 2), "application/json"); emitNotice("Form export downloaded."); };
