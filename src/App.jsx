...
function FormsView({ workspace, update, onNavigate }) {
  ...
  const submitResponse = () => { if (formSettings.collectEmail && !answers.email?.trim()) { emitNotice("Add an email address before submitting."); return; } setSubmitted(true); update({ lastFormResponse: { saved: true, scale, answers } }); emitNotice("Response saved in this local workspace."); };
